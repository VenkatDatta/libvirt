...
static int virLXCDockerSetWorkingDirectory(virDomainDefPtr vmdef,
                                           virJSONValuePtr config)
{
    const char *cwd = virJSONValueObjectGetString(config, "WorkingDir");

    if (strcmp(cwd, "") && VIR_STRDUP(vmdef->os.initdir, cwd) < 0)
        return -1;
